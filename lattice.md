...
## High-level Design
Every section with paragraphs in it defines a concept. For example, suppose a project has this file in `.lat` directory named `tests.md`:
...
2. `lat expand "[prompt]"` -- will convert `[[..]]` references in text to locations and snippets pointing to the information in the `.lat` markdown files. This will be used by coding agents to pre-process user prompts and enrich them with context.
...
5. `lat init` -- will initialize the `.lat` directory with template files
...
The idea is that a project can have `.lat` directory with `.md` files in it, at the same level `.git` directory typically exists.
